...
import IntegerAndSubtraction.sub
...
object IntegerAndSubtractionSpec extends Properties("Integer and subtraction") {
  ...
  property("is total yielding positive result") = forAll(posBigInt, negBigInt) { (a: BigInt, b: BigInt) =>
    val res = sub(a, b)
    res > 0 && res > a && res > b
  }

  property("is total yielding negative result") = forAll(negBigInt, posBigInt) { (a: BigInt, b: BigInt) =>
    val res = sub(a, b)
    res < 0 && res < a && res < b
  }
